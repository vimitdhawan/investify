...
import { calculateXIRRForTransactions, projectCompletionDate } from '@/lib/utils/xirr-calculator';
import { getTransactionsByScemeId, processScheme } from '@/lib/repository/portfolio';
...
async function calculateGoalProjections(
  ...
  const schemesRef = firestore.collection(USERS_COLLECTION).doc(userId).collection(SCHEMES_COLLECTION);
  ...
  if (goal.schemeIds && goal.schemeIds.length > 0) {
    ...
    for (const scheme of fetchedSchemes) {
      const processedScheme = await processScheme(scheme, mostRecentNavDate || new Date()); // Use current date for processing
      if (processedScheme.xirrGainLoss && processedScheme.xirrGainLoss > 0) {
        totalCurrentAmount += processedScheme.marketValue ?? 0;
        const schemeTransactions = await getTransactionsByScemeId(scheme.id);
  ...
  if (validSchemesForXIRR.length > 0 && totalCurrentAmount < goal.targetAmount) {
    ...
    if (combinedXIRR > 0) {
      projectedDate = projectCompletionDate(
        totalCurrentAmount,
        goal.targetAmount,
        mostRecentNavDate || new Date(),
        combinedXIRR
...
export async function createGoal(
  ...
  const { currentAmount, projectedDate } = await calculateGoalProjections(userId, newGoal);
...
export async function getGoals(userId: string): Promise<Goal[]> {
  ...
  const goalsWithProjections = await Promise.all(
    goals.map(async (goal) => {
      const { currentAmount, projectedDate } = await calculateGoalProjections(userId, goal);
...
export async function updateGoal(
  ...
  const { currentAmount, projectedDate } = await calculateGoalProjections(userId, updatedGoal);
