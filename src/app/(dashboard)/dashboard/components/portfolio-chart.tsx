...
export function PortfolioChart({ historicalData }: PortfolioChartProps) {
  const chartData = historicalData.map(pf => ({
    date: new Date(pf.date!).toLocaleDateString('en-US', { month: 'short', year: '2-digit' }),
        ...
        <ChartContainer config={chartConfig} className="min-h-[200px] w-full">
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={chartData} margin={{ left: 20, right: 30, bottom: 60 }}>
                    ...
                    formatter={(value, name) => {
                        ...
                        return (
                            <div className="flex items-center justify-between w-full">
                                ...
                                <span className="ml-4 font-mono font-medium tabular-nums">{formatCurrency(value as number)}</span>
                            </div>
                        )
                    }}
                />}
              ...
              />
              <ChartLegend content={<ChartLegendContent className="mt-4 gap-8"/>} />
            </AreaChart>
          </ResponsiveContainer>
        </ChartContainer>
      </CardContent>
