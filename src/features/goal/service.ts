import { Goal, GoalView } from './type';
import { getGoals, getGoal as getGoalFromRepo } from './repository';
...
export async function fetchGoals(userId: string): Promise<GoalView[]> {
  const [goals, schemes] = await Promise.all([
    getGoals(userId),
    getSchemes(userId),
  ]);

  return goals.map((goal) => {
    const assignedSchemes = schemes.filter((s) =>
...
export async function getGoalView(
  ...
  const [goal, schemes] = await Promise.all([
    getGoalFromRepo(userId, goalId),
    getSchemes(userId),
  ]);
...
function processGoal(goal: Goal, assignedSchemes: Scheme[]): GoalView {
  const currentAmount = assignedSchemes.reduce(
...
function calculateProjectedDate(
  ...
  const positiveXirrSchemes = assignedSchemes.filter(
    (s) => (s.xirrGainLoss || 0) > 0
  );
  ...
  projectedDate.setMonth(
    projectedDate.getMonth() + Math.round(remainingMonths)
  );
