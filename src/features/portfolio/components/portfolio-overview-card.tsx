...
export function PortfolioOverviewCard({
  ...
  const previousDayValueClass = isPreviousDayGain
    ? 'text-[color:var(--gain)]'
    : 'text-[color:var(--loss)]';

  const isAbsoluteGain = portfolio.absoluteGainLoss >= 0;
  const absoluteValueClass = isAbsoluteGain
    ? 'text-[color:var(--gain)]'
    : 'text-[color:var(--loss)]';

  const isRealizedGain = portfolio.realizedGainLoss >= 0;
  const realizedValueClass = isRealizedGain
    ? 'text-[color:var(--gain)]'
    : 'text-[color:var(--loss)]';
    ...
    <Card className="@container/card">
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2 border-t border-dashed pt-2">
          ...
          <FinancialDetail
            label="XIRR"
            value={portfolio.xirrGainLoss}
            isPercentageValue={true}
            iconComponent={IconGauge}
            showIcon={true}
            valueClass={xirrValueClass}
          />
        </div>
